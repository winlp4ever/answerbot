...
def main():
    ...
    es.indices.delete(index='qacoursera', ignore=[400, 404])
    # and create a new one
    with open('./nlp/index.json') as index_file:
        source = index_file.read().strip()
        es.indices.create(index='qacoursera', body=source)
    ...
    while q:
        ...
        es.index(index='qacoursera', body=doc, id=q['id'])
        cnt += 1
...
def verify():
    ...
    res = es.search(index='qacoursera', body=doc)
